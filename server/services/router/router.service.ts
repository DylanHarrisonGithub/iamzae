...
const router = ((): typeof service extends Service ? typeof service : never => {
  const service = async (request: ParsedRequest): ServicePromise<RouterResponse> => {
    ...
    if (!(route.privilege.indexOf('guest') > -1)) {
      ...
      let tokenRes = await server.services.authentication.verifyToken(request.token); // should check for success
      if (!(tokenRes.success)) {
        return new Promise(resolve => resolve({
          success: false,
    ...
    let validation = await server.services.validation(request.params, route.schema);
